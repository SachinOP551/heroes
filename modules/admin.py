...
import asyncio
from modules.infinity_stones import get_infinity_stones_stats
...
@admin_only
async def infinity_stones_stats_command(client: Client, message: Message):
    """Show infinity stones collection statistics"""
    try:
        stats = await get_infinity_stones_stats()
        
        if not stats:
            await message.reply_text("❌ Failed to retrieve infinity stones statistics.")
            return
        
        stats_text = "💎 **INFINITY STONES STATISTICS** 💎\n\n"
        
        # Overall stats
        stats_text += f"📊 **Overall Statistics:**\n"
        stats_text += f"• Total Stones Collected: {stats.get('total_collected', 0):,}\n"
        stats_text += f"• Unique Collectors: {stats.get('unique_collectors', 0):,}\n\n"
        
        # Stone type breakdown
        stats_text += f"🔮 **Stones by Type:**\n"
        stone_stats = stats.get('stone_stats', {})
        for stone_type, count in stone_stats.items():
            stone_name = stone_type.replace('_', ' ').title()
            stats_text += f"• {stone_name}: {count:,}\n"
        
        # Top collectors
        top_collectors = stats.get('top_collectors', [])
        if top_collectors:
            stats_text += f"\n🏆 **Top Collectors:**\n"
            for i, collector in enumerate(top_collectors[:5], 1):
                username = collector.get('username', 'Unknown')
                first_name = collector.get('first_name', 'Unknown')
                stone_count = collector.get('stone_count', 0)
                display_name = f"@{username}" if username else first_name
                stats_text += f"{i}. {display_name}: {stone_count} stones\n"
        
        await message.reply_text(stats_text)
        
    except Exception as e:
        await message.reply_text(f"❌ Error retrieving infinity stones statistics: {e}")
        print(f"Error in infinity_stones_stats_command: {e}")
