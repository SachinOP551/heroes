...
if os.environ.get('USE_POSTGRESQL', 'false').lower() == 'true':
    ...
    from .database import get_database, get_rarity_emoji
...
async def sell_command(client: Client, message: Message):
    ...
    if await action_manager.is_user_busy(db, user_id):
        ...
        if isinstance(action_type, str) and action_type:
            print(f"[DEBUG] Detected action_type for user {user_id}: '{action_type}'")
        else:
            print(f"[DEBUG] No active action for user {user_id} (action_type is None or empty)")
        # Only show active action warning if action_type is a valid string
...
async def handle_masssell_callback(client: Client, callback_query: CallbackQuery):
    ...
    if callback_query.data == "masssell_confirm":
        # Set processing flag IMMEDIATELY to prevent spam clicks
