...
async def massgift_command(client: Client, message: Message):
    ...
    char_docs = await db.characters.find({'character_id': {'$in': char_ids_to_gift}}).to_list(length=None)
...
async def handle_massgift_callback(client: Client, callback_query: CallbackQuery):
    ...
    # Robust: Only allow confirmation if active_action is massgift
    user_data = await db.get_user(user_id)
    ...
    if callback_query.data == "massgift_confirm":
        ...
        try:
            ...
            async with lock:
                try:
                    if active_action.get('type') != 'massgift':
                        ...
                        return
                    if user_id in _massgift_temp and _massgift_temp[user_id].get('completed'):
                    ...
                    try:
                        ...
                        # Remove from sender
                        user_data = await db.users.find_one({'user_id': user_id}, {'characters': 1})
