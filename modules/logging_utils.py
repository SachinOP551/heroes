...
DROPTIME_LOG_CHANNEL = -1002558794123 # New channel for droptime logs
...
async def send_new_user_log(client: Client, user: User):
    ...
    try:
        ...
        print(f"Failed to send new user log (channel may be invalid): {e}")
        # Don't crash the bot, just log the error
...
async def send_new_group_log(client: Client, chat: Chat, added_by: User):
    ...
    log_message = (
        ...
        f"📛 Group: {chat.title}\n"
        f"🆔 Group ID: `{chat.id}`\n"
        f"🔗 Username: {f'@{chat.username}' if chat.username else '`N/A`'}\n\n"
        f"👤 Added By: {added_by.first_name} (@{added_by.username})\n"
        f"🆔 Admin ID: `{added_by.id}`\n"
        f"🔗 Admin Username: {f'@{added_by.username}' if added_by.username else '`N/A`'}\n"
