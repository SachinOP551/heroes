...
async def run_column_migrations():
    ...
    async with _pg_pool.acquire() as conn:
        ...
        await conn.execute('''
            ...
            ADD COLUMN IF NOT EXISTS ban_until TIMESTAMP;
            ALTER TABLE users
            ADD COLUMN IF NOT EXISTS groups INTEGER[] DEFAULT ARRAY[]::INTEGER[];
        ''')
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                if not column_check:
                    ...
                    await conn.execute("""
                        ALTER TABLE users ADD COLUMN groups INTEGER[] DEFAULT ARRAY[]::INTEGER[];
                    """)
                ...
                await conn.execute("""
                    UPDATE users 
                    SET groups = CASE 
                        WHEN groups IS NULL THEN ARRAY[$2::integer] 
                        WHEN NOT ($2 = ANY(groups)) THEN array_append(groups, $2::integer)
                        ELSE groups
                    END
                    WHERE user_id = $1
                """, user_id, group_id)
