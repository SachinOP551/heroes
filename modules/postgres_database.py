...
class PostgresDatabase:
    ...
    async def update_user(self, user_id: int, update_data: dict):
        ...
        from datetime import datetime
        for key, value in update_data.items():
            # Serialize dicts for JSONB columns
            if key in ['active_action', 'collection_preferences'] and isinstance(value, dict):
                value = json.dumps(value)
            if key == 'store_offer' and isinstance(value, dict):
                value = json.dumps(value)
            if key == 'last_propose' and value:
    ...
    async def unban_user(self, user_id: int):
        ...
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE users SET is_banned = FALSE, ban_reason = NULL, ban_until = NULL WHERE user_id = $1",
                user_id
            )
        ...
        try:
            async with self.pool.acquire() as conn:
                rows = await conn.fetch(
                    '''
                    SELECT user_id, first_name AS name, username,
                        (SELECT COUNT(*) FROM unnest(characters) AS c WHERE c = $2) AS count
                    FROM users
                    WHERE $2 = ANY(characters) AND $1::bigint = ANY(groups)
                    ORDER BY count DESC
                    LIMIT 10
...
class PostgresCursor:
    ...
    async def to_list(self, length: int = None):
        ...
        if 'user_id' in self.query:
            ...
            sql = "SELECT * FROM characters WHERE character_id = $1"
            params.append(self.query['character_id'])
        elif 'is_video' in self.query:
            sql = "SELECT * FROM characters WHERE is_video = $1"
            params.append(self.query['is_video'])
        elif 'rarity' in self.query:
            sql = "SELECT * FROM characters WHERE rarity = $1"
...
class PostgresAggregationCursor:
    ...
    async def to_list(self, length: int = None):
        ...
        # This would need to be expanded for complex MongoDB-style aggregations
        return []
