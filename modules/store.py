...
def format_store_message(chars, refreshes, refresh_cost=REFRESH_COST):
    msg = "This Is Your Today's Store:\n"
    for c in chars:
    ...
    msg += "\n"
    if refreshes == 0:
        msg += f"🔄 1 free refresh left today\\.\\({MAX_REFRESHES - refreshes} total left)"
    else:
        left = MAX_REFRESHES - refreshes
        if left > 0:
            msg += f"🔄 Refresh cost: {refresh_cost:,} tokens. ({left} left today)"
        else:
            msg += "❌ No refreshes left today."
    return msg
...
RARITY_WEIGHTS = {
    ...
    "Mythic": 1,
    "Zenith": 1,
    "Premium": 0.5  # Added Premium with weight 1
    # "Supreme" is excluded
}

# List of character IDs to always exclude from store offers
EXCLUDED_CHARACTER_IDS = [531, 664, 678, 849, 853, 877, 957, 1109, 1248, 1305]
...
async def get_weighted_random_characters(db, count=10):
    # Fetch all eligible characters (excluding Supreme and characters with is_video=True) using PostgreSQL
    all_chars = await db.get_all_characters()
    filtered_chars = []
    for char in all_chars:
        if char["rarity"] == "Supreme":
            continue
        # Exclude characters with is_video=True for all rarities
        is_video = char.get("is_video", False)
        if not is_video:
            # Exclude specific character IDs
            char_id = char.get("character_id") or char.get("id") or char.get("_id")
            if char_id not in EXCLUDED_CHARACTER_IDS:
                filtered_chars.append(char)
    if not filtered_chars:
        return []
    return get_weighted_random_characters_sync(filtered_chars, count)
...
async def get_daily_store_offer(db, user_id):
    ...
    if not offer or offer.get("date") != today:
        ...
        offer = {
            "date": today,
            "characters": [c.get("id") or c.get("character_id") or c.get("_id") for c in offer_chars],
            "refreshes": 0,
            "purchased": [],
            "pending_buy": None
        }
    ...
    if "characters" not in offer or not isinstance(offer["characters"], list) or not offer["characters"]:
        ...
        offer_chars = await get_weighted_random_characters(db, 10)
        offer["characters"] = [c.get("id") or c.get("character_id") or c.get("_id") for c in offer_chars]
        await db.update_user(user_id, {"store_offer": offer})
    return offer
