...
RARITY_WEIGHTS = {
    ...
    "Mythic": 1,
    "Zenith": 1,
    "Premium": 1,  # Added Premium with weight 1
    # "Supreme" is excluded
}
...
async def get_weighted_random_characters(db, count=10):
    # Fetch all eligible characters (excluding Supreme and Premium with is_video=True) using PostgreSQL
    all_chars = await db.get_all_characters()
    filtered_chars = []
    for char in all_chars:
        if char["rarity"] == "Supreme":
            continue
        if char["rarity"] != "Premium":
            filtered_chars.append(char)
        else:
            # For Premium, only include those with is_video=False or missing
...
async def get_daily_store_offer(db, user_id):
    ...
    offer = user.get("store_offer") if user else None
    # Defensive: If offer is not a dict, try to parse if string, else reset
    import json
...
async def refresh_store_callback(client: Client, callback_query: CallbackQuery):
    ...
    # If offer is not a dict (e.g., str or None), reset it
    if not isinstance(offer, dict):
        offer = {}
    refreshes = offer.get("refreshes", 0)
