...
from .decorators import admin_only, is_owner, check_banned
...
import mimetypes
import aiohttp
from config import CATBOX_USERHASH  # Add this to your config.py
import logging
import os
...
async def add_character_command(client: Client, message: Message):
    ...
    img_url = await upload_with_fallback(file_id, client)
    if not img_url:
        await processing_msg.edit_text(f"<b>❌ Failed to upload {media_type}.</b>\n\n{get_upload_error_message()}", parse_mode=ParseMode.HTML)
        return
...
async def edit_character_command(client: Client, message: Message):
    ...
    if field == "image" and message.reply_to_message and message.reply_to_message.photo:
        ...
        img_url = await upload_with_fallback(file_id, client)
        if not img_url:
            await message.reply_text(f"<b>❌ Failed to upload image.</b>\n\n{get_upload_error_message()}", parse_mode=ParseMode.HTML)
            return
...
async def upload_to_imgbb(file_id: str, client) -> str:
    """Upload image to ImgBB as fallback when Catbox is unavailable."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                ...
                # Note: ImgBB requires an API key for uploads
                # For now, we'll use a public demo key, but you should get your own from https://api.imgbb.com/
                api_key = "0e41b21a7b6202d84e85ed9fd7d300ff"  # Demo key - replace with your own
...
async def upload_with_fallback(file_id: str, client) -> str:
    """Upload image with fallback from Catbox to ImgBB if Catbox fails."""
    
    print("🔄 Attempting to upload to Catbox...")
    result = await upload_to_catbox(file_id, client)
    
    if result:
        print("✅ Successfully uploaded to Catbox!")
        return result
    
    print("❌ Catbox upload failed, trying ImgBB as fallback...")
    result = await upload_to_imgbb(file_id, client)
    
    if result:
        print("✅ Successfully uploaded to ImgBB!")
        return result
    
    print("❌ Both Catbox and ImgBB uploads failed!")
    return None
...
# Replace with your actual Catbox userhash (from https://catbox.moe/user/manage.php)
CATBOX_USERHASH = 'fff711c3a6ad5a2b232a50a4f'

async def upload_to_catbox(file_id: str, client) -> str:
    """Upload image to Catbox using public API with optional userhash."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
                ...
                for endpoint in endpoints:
                    try:
                        async with session.post(endpoint, data=data) as response:
                            if response.status != 200:
                                print(f"Catbox API returned status {response.status} for endpoint {endpoint}")
                                continue
                            ...
                            if result and not result.startswith('Error'):
                                ...
                                print(f"Catbox upload failed for endpoint {endpoint}: {result or 'Unknown error'}")
                                continue
                ...
                raise Exception("All Catbox endpoints failed")
