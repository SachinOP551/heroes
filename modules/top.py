...
async def tdtop_command(client: Client, message: Message):
    ...
    try:
        ...
        for user in users:
            user_id = user['user_id']
            collection_history = user.get('collection_history', [])
            print(f"[DEBUG] User {user_id} collection_history: {collection_history}")  # DEBUG
            ...
            for entry in collection_history:
                try:
                    ...
                    print(f"[DEBUG] Entry: {entry}")  # DEBUG
                    if not collected_at:
                        continue
                    from dateutil.parser import parse
                    collected_at_dt = parse(collected_at)
                    print(f"[DEBUG] Parsed collected_at: {collected_at_dt}")  # DEBUG
                    ...
                    print(f"[DEBUG] collected_at_ist: {collected_at_ist}, today_ist: {today_ist}, tomorrow_ist: {tomorrow_ist}")  # DEBUG
                    if today_ist <= collected_at_ist < tomorrow_ist and entry.get('source', 'collected') == 'collected':
                        today_count += 1
                except Exception as e:
                    print(f"[DEBUG] Error parsing entry: {e}")  # DEBUG
                    continue
            if today_count > 0:
                collectors.append({
                    'first_name': user.get('first_name', 'Unknown'),
