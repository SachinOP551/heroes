            ...
            for entry in collection_history:
                try:
                    ...
                    if collected_at.tzinfo is None:
                        in_range = today <= collected_at.replace(tzinfo=ist) < tomorrow
                    else:
                        in_range = today <= collected_at.astimezone(ist) < tomorrow
...
async def top_command(client: Client, message: Message):
    ...
    db = get_database()
    chat = message.chat
    print(f"[DEBUG] chat.type: {chat.type}, type: {type(chat.type)}, chat.id: {chat.id}, chat.title: {getattr(chat, 'title', None)}")
