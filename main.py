...
from config import TOKEN, API_ID, API_HASH, LOG_CHANNEL_ID, DROPTIME_LOG_CHANNEL, OWNER_ID, NEON_URI
import asyncio
from pyrogram import Client, filters
from pyrogram.types import Message, CallbackQuery, InlineQuery, BotCommand


# --- EARLY GLOBAL PATCH: Ensure all get_database calls auto-initialize the database ---
import modules.postgres_database as pg_db
import sys as _sys
_original_get_database = pg_db.get_database
def patched_get_database(*args, **kwargs):
    try:
        return _original_get_database(*args, **kwargs)
    except Exception:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            # Try to initialize the database asynchronously
            coro = pg_db.init_database(NEON_URI)
            fut = asyncio.run_coroutine_threadsafe(coro, loop)
            fut.result(timeout=5)
        else:
            loop.run_until_complete(pg_db.init_database(NEON_URI))
        return _original_get_database(*args, **kwargs)
# Patch everywhere
pg_db.get_database = patched_get_database
_sys.modules['modules.postgres_database'].get_database = patched_get_database
# Patch the symbol in this module as well
get_database = patched_get_database
# --- END EARLY GLOBAL PATCH ---

# Global flag to track database initialization
_database_initialized = False

# Always use PostgreSQL database imports
from modules.postgres_database import init_database, ensure_database, get_performance_stats, clear_all_caches, close_database, get_database
import modules.postgres_database as pg_db
...
from modules.suggest import suggest_command, suggest_callback
from modules.fusion import fuse_command, fusion_info_command
from modules.achievement import achievement_command, achievement_callback
from modules.tdgoal import tdgoal_command, tdgoal_callback, track_collect_drop
from modules.auction import auction_command, bid_command, auctions_command, auction_view_callback, auctionview_back_callback, cancel_auction_command
from modules.broadcast import broadcast_command
from modules.start import start_command, new_chat_members, help_callback, back_callback
from modules.giveaway import start_giveaway, enter_giveaway, end_giveaway, giveaway_status
from modules.check import check_command, collectors_here_callback, top_collectors_callback, back_to_character_callback
...
from modules.bang import bang_command, unbang_command, baninfo_command
from modules.srarity import srarity_command, rarity_callback
from modules.stats import stats_command
from modules.search import search_command, inline_query_handler
from modules.tokens import (
    balance_command, deposit_command, withdraw_command,
    daily_command, weekly_command, monthly_command,
    give_tokens, take_tokens, pay_command, shards_pay,
    give_shards, take_shards,  dart_command, basket_command, roll_command, football_command
)
from modules.give import (
    give_command, take_command, massgive_command, give_take_massgive_callback
)
from modules.status import status_command
from modules.propose import propose_command, proposelock_command, setcooldown_command, setcost_command, setacceptance_command, pconfig_command, prate_command, propose_callback, ping_command, pweights_command
from modules.redeem import redeem_command, credeem_command, tredeem_command, sredeem_command
from modules.top import (
    tdtop_command, gtop_command, top_command, rgtop_command, btop_command,
    test_leaderboard_command, sgtop_command
)
from modules.favorite import favorite_command, handle_favorite_callback
from modules.trade import (
    gift_command, handle_gift_callback,
    trade_command, handle_trade_callback,
    handle_cancel_callback,
    massgift_command, handle_massgift_callback
)
from modules.sell import (
    sell_command, handle_sell_callback, masssell_command, handle_masssell_callback
)
from modules.transfer import (
    transfer_command, handle_transfer_callback
)
from modules.upload import (
    add_character_command, add_character_confirm_callback, add_character_cancel_callback,
    edit_character_command, edit_character_confirm_callback, edit_character_cancel_callback,
    delete_character_command, delete_character_confirm_callback, delete_character_cancel_callback,
    reset_character_command, reset_character_confirm_callback, reset_character_cancel_callback
)
from modules.drop import (
    drop_command, collect_command, droptime_command, free_command, handle_message,
    user_msgs, set_all_droptime_command, clear_banned_command, clear_proposes_command,
    jackpot_command
)
from modules.collection import (
    collection_command, smode_command, handle_smode_callback, handle_collection_callback,
    handle_inline_query
)
from modules.vid import vadd_command, vedit_command
from modules.vidcollection import vidcollection_command, handle_vidcollection_pagination, vidlist_command, handle_vidlist_pagination
from modules.drop_settings import (
    drop_settings_command, lock_rarity_command, unlock_rarity_command,
    set_frequency_command, set_daily_limit_command, drop_settings_callback
)
from modules.drop_weights import setup_drop_weights_and_limits
from modules.store import (
    mystore_command, buy_command, refresh_store_callback, confirm_buy_callback,
    cancel_buy_callback, refresh_all_stores_command, buy_from_store_callback, handle_id_input
)
from modules.suggest import suggest_command, suggest_callback
from modules.fusion import fuse_command, fusion_info_command
import random
...
# Simple connection management - no performance optimizer needed
