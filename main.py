...
async def startup_initialization():
    ...
    # Initialize infinity stones system
    try:
        await initialize_infinity_stones()
    except Exception as e:
        print(f"⚠️ Warning: Failed to initialize infinity stones system: {e}")
...
async def start_handler(client: Client, message: Message):
    ...
    commands = [
        ...
        BotCommand("inventory", "View your item inventory")
    ]
...
async def handle_id_input_handler(client: Client, message: Message):
    """Handle character ID input for both regular store and supremestore purchases"""
    user_id = message.from_user.id
    
    # Check if user is waiting for supremestore ID input first
    if user_id in waiting_for_supreme_store_id:
        # Remove from waiting list
        del waiting_for_supreme_store_id[user_id]
        
        # Process the ID input for supremestore
        char_id = int(message.text)
        await process_supreme_store_buy_request(client, message, char_id)
        return
    
    # If not waiting for supremestore, check if waiting for regular store
    if user_id in waiting_for_id:  # This is for regular store
        # Let the regular store handler deal with it
        await handle_id_input(client, message)
        return
    
    # If not waiting for either store, ignore the input
    return
...
@app.on_message(filters.command("infinitystats", prefixes=["/", ".", "!"]) & filters.user(OWNER_ID))
async def infinity_stats_handler(client: Client, message: Message):
    await infinity_stones_stats_command(client, message)
...
@app.on_callback_query(filters.regex(r"^collect_stone_|^refresh_infinity_stones"))
async def infinity_stones_callback_handler(client: Client, callback_query: CallbackQuery):
    await handle_infinity_stones_callback(client, callback_query)
...
## /infinitystones command removed

@app.on_message(filters.command("collect", prefixes=["/", ".", "!"]))
@auto_register_user
async def collect_stone_handler(client: Client, message: Message):
    await collect_stone_command(client, message)

@app.on_message(filters.command("inventory", prefixes=["/", ".", "!"]))
@auto_register_user
async def inventory_handler(client: Client, message: Message):
    await inventory_command(client, message)

@app.on_message(filters.command("craftgauntlet", prefixes=["/", ".", "!"]))
@auto_register_user
async def craft_gauntlet_handler(client: Client, message: Message):
    await craft_gauntlet_command(client, message)


@app.on_message(filters.command("checkgroup", prefixes=["/", ".", "!"]))
@auto_register_user
async def check_group_access_handler(client: Client, message: Message):
    await check_group_access_command(client, message)
 
@app.on_callback_query(filters.regex(r"^forge_gauntlet_(confirm|cancel)_\d+$"))
async def gauntlet_confirmation_handler(client: Client, callback_query: CallbackQuery):
    from modules.infinity_stones import handle_gauntlet_confirmation
    await handle_gauntlet_confirmation(client, callback_query)
...
@app.on_message(filters.command("supremestore", prefixes=["/", ".", "!"]))
@auto_register_user
async def supreme_store_handler(client: Client, message: Message):
    await supremestore_command(client, message)

@app.on_callback_query(filters.regex(r"^buy_from_supreme_store$"))
async def buy_from_supreme_store_callback_handler(client: Client, callback_query: CallbackQuery):
    await buy_from_supreme_store_callback(client, callback_query)

@app.on_callback_query(filters.regex(r"^refresh_supreme_store$"))
async def refresh_supreme_store_callback_handler(client: Client, callback_query: CallbackQuery):
    await refresh_supreme_store_callback(client, callback_query)

@app.on_callback_query(filters.regex(r"^confirm_supreme_buy_(\d+)$"))
async def confirm_supreme_buy_callback_handler(client: Client, callback_query: CallbackQuery):
    await confirm_supreme_buy_callback(client, callback_query)

@app.on_callback_query(filters.regex(r"^cancel_supreme_buy_(\d+)$"))
async def cancel_supreme_buy_callback_handler(client: Client, callback_query: CallbackQuery):
    await cancel_supreme_buy_callback(client, callback_query)
